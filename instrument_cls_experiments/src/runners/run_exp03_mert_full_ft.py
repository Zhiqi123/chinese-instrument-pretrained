"""
Phase 3 MERT full fine-tuning single seed x ratio runner.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Phase 3 MERT full fine-tuning single run")
...
def main():
    ...
    print(f"=== [Phase 3] MERT Full Fine-Tuning — {ratio_name} × {seed_name} "
    ...
    update_registry(
        ...
        task_type="full_finetune",
        model="mert_v1_95m",
        notes=f"Phase 3; ratio={ratio_name}; seed={base_seed}; full fine-tuning",
    )

    print(f"  [Phase 3] MERT full fine-tuning {ratio_name} × {seed_name}: DONE")
