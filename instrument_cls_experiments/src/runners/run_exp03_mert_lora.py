"""
Phase 3 MERT + LoRA single seed x ratio runner.
Shared by rank=4 and rank=8 via method_short in config YAML.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Phase 3 MERT + LoRA single run")
...
def _merge_training_config(adapt_cfg: dict, method_cfg: dict) -> dict:
    """Merge default_adaptation.yaml with method config training overrides."""
...
def main():
    ...
    subsample_cfg = _load_yaml(EXP_ROOT / method_cfg["subset_config"])
    ...
    print(f"=== [Phase 3] MERT + LoRA (rank={rank}) — {ratio_name} × {seed_name} "
    ...
    # Output directory
    output_dir = EXP_ROOT / method_cfg["output"]["base_dir"] / ratio_name / seed_name
    ...
    # Train loader uses subsetted filtered_df
    ...
    # Val loader uses full unified_df
    ...
    for split in ["val", "test", "external_test"]:
        ...
        meta_loader = build_dataloader(unified_df, split, batch_size=eval_batch, num_workers=0)
        meta_list = []
        true_ids = []
        ...
        assert len(meta_list) == len(pred_ids), \
            f"meta count {len(meta_list)} != pred count {len(pred_ids)}"
    ...
    update_registry(
        ...
        task_type="lora",
        model="mert_v1_95m",
        notes=f"Phase 3; ratio={ratio_name}; seed={base_seed}; LoRA rank={rank}",
    )

    print(f"  [Phase 3] MERT + LoRA (rank={rank}) {ratio_name} × {seed_name}: DONE")
