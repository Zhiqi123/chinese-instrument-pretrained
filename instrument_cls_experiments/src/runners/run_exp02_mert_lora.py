# Exp 2: MERT + LoRA single-seed run. Shared by rank=4 and rank=8.
...
def parse_args():
    parser = argparse.ArgumentParser(description="MERT + LoRA single-seed runner")
...
def main():
    ...
    print(f"=== [Phase 2] MERT + LoRA (rank={rank}) — {seed_name} (seed={base_seed}) ===")
    ...
    for split in ["val", "test", "external_test"]:
        ...
        # Collect metadata (num_workers=0 for deterministic order)
    ...
    update_registry(
        ...
        task_type="lora",
        model="mert_v1_95m",
        notes=f"Phase 2; seed={base_seed}; LoRA rank={rank}",
    )

    print(f"  [Phase 2] MERT + LoRA (rank={rank}) {seed_name}: DONE")
